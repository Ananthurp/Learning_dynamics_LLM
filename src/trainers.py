...
# except Exception:
#     tp = None  # Only needed if you select TensorParallelTrainer
# import contextlib
# import pdb
...
#     """
#     Sparsemax (Martins & Astudillo, 2016). Works like softmax but yields sparse probabilities.
#     logits: (..., V)
#     returns: (..., V), non-negative, sums to 1 along dim, many exact zeros.
#     """
#     z = logits
#     # sort descending
#     z_sorted, _ = torch.sort(z, dim=dim, descending=True)
#     # cumulative sum of sorted scores
#     z_cumsum = torch.cumsum(z_sorted, dim=dim)

#     # k: number of active entries
#     dims = z.size(dim)
#     rng = torch.arange(1, dims + 1, device=z.device, dtype=z.dtype)
#     # shape rng to broadcast on 'dim'
#     view = [1] * z.dim()
#     view[dim] = -1
...
#     Args:
#       logits: (B, M, V)
#       labels: (B, M), with -100 masked positions (prompt tokens)

#     Returns:
#       scores: (B,) where score = - sum_l [ 0.5*||p_l||_2^2 - z_l[y_l] + 0.5 ] over unmasked tokens
#               with p_l = sparsemax(z_l)
#     """
...
#     z = logits[:, :-1, :]
#     mask = (y != -100)
#     # dummy value for masked positions
#     y[y == -100] = 0
...
#     # NOTE: stop-gradient happens at DPO head; here we just return scores
...
#         if tp is None:
#             raise ImportError('tensor_parallel failed to import; install a compatible version or use BasicTrainer/FSDPTrainer')
...
#     logits = pi_logratios - ref_logratios  # h_{πθ}^{y_w,y_l}

#     if ipo:
#         losses = (logits - 1/(2 * beta)) ** 2
#     else:
#         # Original DPO with optional label smoothing
#         losses = -F.logsigmoid(beta * logits) * (1 - label_smoothing) - F.logsigmoid(-beta * logits) * label_smoothing
...
# def _get_batch_logps(logits: torch.FloatTensor, labels: torch.LongTensor, average_log_prob: bool = False) -> torch.FloatTensor:
#     """Compute the log probabilities over the label sequence, with standard shift/mask handling."""
#     assert logits.shape[:-1] == labels.shape
...
#     per_token_prob_argmax = torch.exp(per_token_logps_argmax)
#     per_token_prob_exceptargmax = torch.ones_like(per_token_prob_argmax) * loss_mask - per_token_prob_argmax * loss_mask
#     per_token_logp_exceptargmax = torch.log(per_token_prob_exceptargmax + 1e-100)

#     prob_norm = torch.linalg.vector_norm(prob_logits, ord=2, dim=-1)
#     prob_norm = prob_norm * loss_mask
#     prob_norm2_mean = torch.square(prob_norm.sum(-1) / loss_mask.sum(-1))
#     A_norm = torch.sqrt(V * prob_norm2_mean + (V - 2) * torch.ones_like(prob_norm2_mean))

#     e_oht = torch.nn.functional.one_hot(labels, num_classes=V)
#     prob_gap_norm = torch.linalg.vector_norm(prob_logits - e_oht, ord=2, dim=-1)
#     prob_gap_norm = prob_gap_norm * loss_mask
#     prob_gap2_mean = prob_gap_norm.sum(-1) / loss_mask.sum(-1)

#     prob_label = torch.gather(prob_logits, dim=2, index=labels.unsqueeze(2)).squeeze(2)
#     prob_label_gap = torch.ones_like(prob_label) - prob_label
#     prob_energy = (prob_label_gap * loss_mask).sum(-1) / loss_mask.sum(-1)
...
# def concatenated_inputs(batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
#     """Concatenate the chosen and rejected inputs into a single tensor to share a forward pass."""
#     max_length = max(batch['chosen_input_ids'].shape[1], batch['rejected_input_ids'].shape[1])
#     concatenated_batch = {}
#     for k in batch:
#         if k.startswith('chosen') and isinstance(batch[k], torch.Tensor):
...
# # --- SP-DPO: compute FY scores for concatenated forward -------------------
# def concatenated_fy_scores(model: nn.Module,
#                            batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
#     """Forward once on concatenated chosen/rejected and compute FY scores per sequence.
#        Returns (chosen_scores, rejected_scores), each shape (batch_size,).
#     """
#     concat = concatenated_inputs(batch)
#     all_logits = model(
#         concat['concatenated_input_ids'],
#         attention_mask=concat['concatenated_attention_mask']
#     ).logits.to(torch.float32)
#     all_scores = _get_batch_fy_scores(all_logits, concat['concatenated_labels'])
...
#         self.config = config
#         self.run_dir = run_dir
#         self.prob_dicts = ['chosen', 'chosen_initial', 'chosen_gptsemantic', 'chosen_gptformat',
#                            'rejected', 'reject_gptsemantic', 'reject_gptformat',
#                            'irr_train', 'irr_test', 'irr_hum',
#                            'random_permute', 'random_nonhum']
...
#     ) -> Tuple[str, str]:
#         """Generate samples from the policy for the given batch of inputs."""
#         # FSDP generation according to official guidance
#         ctx = lambda: (
#             FSDP.summon_full_params(
#                 self.policy, writeback=False, recurse=False
#             ) if 'FSDP' in self.config.trainer else contextlib.nullcontext()
...
#         # include sp_dpo for symmetry (sampling reference if present)
...
#     ) -> Tuple[torch.FloatTensor, Dict[str, List], np.ndarray]:
#         """Compute the loss and metrics for the given batch (SFT, DPO, IPO, SP-DPO)."""

#         metrics: Dict[str, List] = {}
#         train_test = 'train' if train else 'eval'
        
#         chosen = 'chosen' if self.config.train_supervise is None else self.config.train_supervise
#         argmax_token = np.array([0])  # placeholder default

#         if train:
#             # --- DPO / IPO / SP_DPO branch
#             if loss_config.name in {'dpo', 'ipo', 'sp_dpo'} and not force_sft:
#                 if loss_config.name == 'sp_dpo':
#                     # Sparsemax-DPO via FY scores
#                     policy_chosen_scores, policy_rejected_scores = concatenated_fy_scores(self.policy, batch)
#                     if getattr(loss_config, 'reference_free', False):
#                         reference_chosen_scores = torch.zeros_like(policy_chosen_scores)
#                         reference_rejected_scores = torch.zeros_like(policy_rejected_scores)
#                     else:
#                         with torch.no_grad():
#                             reference_chosen_scores, reference_rejected_scores = concatenated_fy_scores(self.reference_model, batch)

#                     delta_theta = policy_chosen_scores - policy_rejected_scores
#                     delta_ref = reference_chosen_scores - reference_rejected_scores
#                     margins = (delta_theta - delta_ref).detach()

#                     # SP-DPO loss in score space
#                     m = loss_config.beta * (delta_theta - delta_ref)
#                     losses = -F.logsigmoid(m)

#                     # for compatibility with downstream logging keys; reuse names locally
#                     policy_chosen_logps = policy_chosen_scores
#                     policy_rejected_logps = policy_rejected_scores

#                     # record FY metrics
#                     policy_chosen_scores_g = all_gather_if_needed(policy_chosen_scores.detach(), self.rank, self.world_size)
#                     policy_rejected_scores_g = all_gather_if_needed(policy_rejected_scores.detach(), self.rank, self.world_size)
#                     margins_g = all_gather_if_needed(margins, self.rank, self.world_size)
#                     metrics[f'fy_{train_test}/chosen'] = policy_chosen_scores_g.cpu().numpy().tolist()
#                     metrics[f'fy_{train_test}/rejected'] = policy_rejected_scores_g.cpu().numpy().tolist()
#                     metrics[f'fy_{train_test}/margins'] = margins_g.cpu().numpy().tolist()

#                     # also expose scores_* for plotting parity
#                     metrics[f'scores_{train_test}/chosen'] = policy_chosen_scores_g.cpu().numpy().tolist()
#                     metrics[f'scores_{train_test}/rejected'] = policy_rejected_scores_g.cpu().numpy().tolist()

#                     # expose "rewards" in score space for parity with DPO logs
#                     chosen_rewards = loss_config.beta * (policy_chosen_scores - reference_chosen_scores)
#                     rejected_rewards = loss_config.beta * (policy_rejected_scores - reference_rejected_scores)
#                     metrics[f'rewards_{train_test}/chosen'] = all_gather_if_needed(chosen_rewards.detach(), self.rank, self.world_size).cpu().numpy().tolist()
#                     metrics[f'rewards_{train_test}/rejected'] = all_gather_if_needed(rejected_rewards.detach(), self.rank, self.world_size).cpu().numpy().tolist()
#                     metrics[f'rewards_{train_test}/margins'] = all_gather_if_needed((chosen_rewards - rejected_rewards).detach(), self.rank, self.world_size).cpu().numpy().tolist()

#                 else:
#                     # original DPO/IPO path
#                     policy_chosen_logps, policy_rejected_logps = self.concatenated_forward(self.policy, batch)
#                     with torch.no_grad():
#                         reference_chosen_logps, reference_rejected_logps = self.concatenated_forward(self.reference_model, batch)
...
#                     policy_rejected_logps_g = all_gather_if_needed(policy_rejected_logps.detach(), self.rank, self.world_size)
#                     metrics[f'logps_{train_test}/rejected'] = policy_rejected_logps_g.cpu().numpy().tolist()
#                     argmax_token = np.array([-1])
...
#             policy_chosen_logps_g = all_gather_if_needed(
#                 policy_chosen_logps.detach(), self.rank, self.world_size
#             )
#             metrics[f'logps_{train_test}/chosen'] = policy_chosen_logps_g.cpu().numpy().tolist()
...
#                         del policy_predict_logtis
#                         metrics[f'{k}_A_o'] = policy_argmax_logps[2].cpu().numpy().tolist()
#                         metrics[f'logps_{train_test}_{prob_set}/{k}'] = policy_predict_logps.cpu().numpy().tolist()
#                         if k == 'chosen':
#                             metrics[f'argmax_prob_logits'] = policy_argmax_logps[0].cpu().numpy().tolist()
#                             metrics[f'except_argmax_prob_logits'] = policy_argmax_logps[1].cpu().numpy().tolist()
#                             metrics[f'p_e'] = policy_argmax_logps[3].cpu().numpy().tolist()
...
#             with torch.no_grad():
#                 _, eval_metrics, argmax_token = self.get_batch_metrics(local_eval_batch, self.config.loss, train=False, prob_set=prob_set)
#             all_argmax_token.append(argmax_token)
#             for k, v in eval_metrics.items():
#                 all_eval_metrics[k].extend(v)
...
#             local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank, self.world_size, self.rank)
#             with torch.no_grad():
#                 policy_samples, _ = self.get_batch_samples(local_eval_batch, sample_flag=True)
#                 for prompt, sample in zip(eval_batch['prompt'], policy_samples):
#                     all_policy_samples.append({'prompt': prompt, 'response': sample})
#         output_dir = os.path.join(self.config.save_path, f'{prob_set}_response.jsonl')
#         with open(output_dir, 'a', newline='\n') as f:
...
#             if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
#                 rank0_print(f'Running evaluation after {self.example_counter} train examples')
#                 logp_npy, argmax_npy = self.evaluation(prob_set='prob_train')
#                 if self.rank == 0 and self.config.fine_evaluation:
#                     logp_npy_all.append(logp_npy)
#                     argmax_npy_all.append(argmax_npy)
#             #### END EVALUATION ####
...
#         # --- Save fine-eval arrays if requested
#         if self.config.fine_evaluation and self.rank == 0:
#             output_dir = os.path.join(self.config.save_path, f'logp_npy_all_{self.config.train_supervise}.npy')
#             output_dir_argmax = os.path.join(self.config.save_path, f'argmax_token_all_{self.config.train_supervise}.npy')
#             np.save(output_dir, np.array(logp_npy_all))
#             np.save(output_dir_argmax, np.array(argmax_npy_all, dtype=object), allow_pickle=True)
...
#     def clip_gradient(self):
#         """Clip the gradient norm of the parameters of an FSDP policy, gathering gradients across all GPUs."""
#         return self.policy.clip_grad_norm_(self.config.max_grad_norm).item()

#     def save(self, output_dir=None, metrics=None):
#         """Save policy, optimizer, and scheduler state to disk (rank-0 only for full state dicts)."""
#         save_policy = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
#         with FSDP.state_dict_type(
#             self.policy, StateDictType.FULL_STATE_DICT,
#             state_dict_config=save_policy
...
def concatenated_entmax_fy_scores(
    model: nn.Module,
    batch: Dict[str, Union[List, torch.LongTensor]],
    alpha: float = 1.5,
    length_normalize: bool = False,
    ...
    """
    Single forward for chosen+rejected; return FY scores (entmax_α) per sequence.
    """
    concat = concatenated_inputs(batch)
    all_logits = model(
        concat['concatenated_input_ids'],
    ...
    all_scores = _fy_sequence_scores_entmax(
        all_logits, concat['concatenated_labels'],
        alpha=alpha, length_normalize=length_normalize
    )
    B = batch['chosen_input_ids'].shape[0]
    chosen_scores = all_scores[:B]
    rejected_scores = all_scores[B:]
...
class BasicTrainer(object):
    ...
    def get_batch_metrics(
        ...
        """Compute the loss and metrics for the given batch (SFT, DPO, IPO, SP-DPO, ENDPO)."""
        ...
        if train:
            # --- DPO / IPO / SP_DPO / ENDPO branch
            if loss_config.name in {'dpo', 'ipo', 'sp_dpo', 'endpo'} and not force_sft:
                if loss_config.name == 'sp_dpo':
                    # Sparsemax-DPO via FY scores
                    policy_chosen_scores, policy_rejected_scores = concatenated_fy_scores(self.policy, batch)
                    if getattr(loss_config, 'reference_free', False):
                ...
                elif loss_config.name in {'dpo', 'ipo'}:
                    # original DPO/IPO path
                    policy_chosen_logps, policy_rejected_logps = self.concatenated_forward(self.policy, batch)
                    with torch.no_grad():
                        reference_chosen_logps, reference_rejected_logps = self.concatenated_forward(self.reference_model, batch)
                ...
                elif loss_config.name == 'endpo':
                    # Entmax-DPO via FY scores with margin-limited softplus
                    alpha = float(loss_config.alpha)
                    length_norm = bool(getattr(loss_config, 'length_normalize', False))

                    # policy FY scores
                    policy_chosen_scores, policy_rejected_scores = concatenated_entmax_fy_scores(
                        self.policy, batch, alpha=alpha, length_normalize=length_norm
                    )

                    # reference FY scores (or reference-free)
                    if getattr(loss_config, 'reference_free', False):
                        reference_chosen_scores = torch.zeros_like(policy_chosen_scores)
                        reference_rejected_scores = torch.zeros_like(policy_rejected_scores)
                    else:
                        with torch.no_grad():
                            reference_chosen_scores, reference_rejected_scores = concatenated_entmax_fy_scores(
                                self.reference_model, batch, alpha=alpha, length_normalize=length_norm
                            )
                    ...
                    m = beta * (delta_theta - delta_ref)

                    # margin-limited softplus: log(1 + exp(-(m - γ)/τ))
                    gamma = float(loss_config.gamma)
                    tau = float(loss_config.tau)
                    losses = F.softplus(-(m - gamma) / tau)

                    # logging similar to SP-DPO
                    policy_chosen_scores_g = all_gather_if_needed(policy_chosen_scores.detach(), self.rank, self.world_size)
                    policy_rejected_scores_g = all_gather_if_needed(policy_rejected_scores.detach(), self.rank, self.world_size)
                    margins_g = all_gather_if_needed(margins, self.rank, self.world_size)
                    metrics[f'fy_{train_test}/chosen'] = policy_chosen_scores_g.cpu().numpy().tolist()
                    ...
                    # compatibility with downstream logging that expects these names
                    policy_chosen_logps = policy_chosen_scores
                    policy_rejected_logps = policy_rejected_scores
    ...
    def train(self):
        ...
        if self.config.loss.name in {'dpo', 'ipo', 'sp_dpo', 'endpo'}:
            self.reference_model.eval()
...
class FSDPTrainer(BasicTrainer):
    def __init__(
        ...
        if config.loss.name in {'dpo', 'ipo', 'sp_dpo', 'endpo'}:
            rank0_print('Sharding reference model...')
            self.reference_model = FSDP(reference_model, **shared_fsdp_kwargs)
